...
var Observer = function Observer (value) {
  ...
  this.dep = new Dep();
  this.vmCount = 0;
  def(value, '__ob__', this); // 给传入的属性新增属性 __ob__ 标记，值为当前的 Observer 对象，表示已注册过 Observer
...
function mergeDataOrFn (
  ...
  if (!vm) {
    ...
    return function mergedInstanceDataFn () {
      // instance merge
      var instanceData = typeof childVal === 'function' // 获取实例的数据
        ? childVal.call(vm, vm)
        : childVal;
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        // Start tag:
        var startTagMatch = parseStartTag(); // 解析开始的标签，把 template 转化成节点名和属性
        if (startTagMatch) {
...
function createCompileToFunctionFn (compile) {
  ...
  return function compileToFunctions (
    ...
    // compile
    var compiled = compile(template, options); // 将模板与编译配置传入 compile 函数
...
function createCompilerCreator (baseCompile) { // 创建一个编译器构造器，用于传递
...
var activeInstance = null; // 全局变量，指当前上下文的 vue 实例
...
function installComponentHooks (data) {
  ...
  for (var i = 0; i < hooksToMerge.length; i++) {
    var key = hooksToMerge[i]; // 各钩子函数名
    var existing = hooks[key]; // hook 里边取
    var toMerge = componentVNodeHooks[key]; // 定义里边取
